...
const exportedMethods = {
	...
	async createCustomer(result) {
		...
		objKeys.forEach((element) => {
			result[element] = helpers.checkInput(element, result[element], element + " of the customer");
		});
		...
		if (result.google_authenticated && result.google_authenticated == 1) {
			...
			if (duplicateUser !== null) {
				await customerCollection.updateOne({ email: result.email, google_authenticated: 2 }, { $set: { google_authenticated: 1, password: "" } });
