	...
	const createBusinessAdmin = async () => {
		...
		try {
			...
			objKeys.forEach((element) => {
				payload[element] = helpers.checkInput(element, payload[element], element + " of the admin", true);
			});
			...
			await axios.post("/admin/register-business-admin", formData, payloadHeader);
			setLoading(false);
			setName("");
			setBusinessName("");
	...
	return (
		<div className="flex justify-center h-full">
			...
			<div className="bg-white flex flex-col w-full md:py-8 my-auto justify-center items-center rounded">
				<div className="text-gray-900 text-lg mb-1 font-medium title-font">Create a Business Admin</div>

				<FormInput title="AdminName" type="text" value={name} changeAction={setName} />
				...
				{!loading ? (
					...
					<Button title="Creating...." disabled={true} color="gray" clickAction={createBusinessAdmin} />
				)}

				<TimeoutComponent show={showCreated} setShow={setShowCreated}>
					<div className="text-green-700 text-lg mb-1 mt-5 font-medium title-font">Business Created!!</div>
				</TimeoutComponent>
			</div>
		</div>
	);
