...
router.route("/generate_coupon").post(async (req, res) => {
	try {
		...
		if (!req.session.admin_role || !req.session.admin_role == process.env.BUSINESS_ADMIN_ROLE) {
		...
		objKeys.forEach((element) => {
			helpers.checkInput(element, result[element], element + " of the coupon", true);
		});
...
router.route("/coupons").get(async (req, res) => {
	try {
		...
		if (!req.session.admin_role || !req.session.admin_role == process.env.MASTER_ADMIN_ROLE) {
			errorObject.status = 403;
			errorObject.message = "Unauthorized Access";
			throw errorObject;
		}
...
router.route("/update-coupon-status/:business_id/:coupon_id").get(async (req, res) => {
	try {
		const errorObject = {
			status: 400,
		};
		...
		business_id = helpers.checkInput("business_id", business_id, "Invalid Business ID");
		coupon_id = helpers.checkInput("coupon_id", coupon_id, "Invalid Coupon ID");
		let couponList = await couponsData.updateCouponStatus(business_id, coupon_id);
...
router.route("/get-proof/:business_id").get(async (req, res) => {
	try {
		...
		let business_id = req.params.business_id;
		business_id = helpers.checkInput("business_id", business_id, "Business ID of the business", true);
