	...
	const checkCredentialsAndLogin = async () => {
		...
		if (adminType === "master") {
			try {
				...
				const trimmedUsername = helpers.checkInput("email", email, "username" + " of the master admin", true);

				const trimmedPassword = helpers.checkInput("password", password, "password" + " of the master admin", true);
			...
			try {
				...
				objKeys.forEach((element) => {
					adminCreds[element] = helpers.checkInput(element, adminCreds[element], element + " of the customer", true);
				});
	...
	return (
		<div className="flex justify-center h-full">
			...
			<div className="lg:w-1/3 md:w-1/2 bg-white bg-opacity-30 flex flex-col w-full my-auto md:py-8 items-center rounded shadow-2xl ">
				<div className="text-gray-900 text-lg mb-1 font-medium title-font">Admin Login</div>

				<FormInput title="Username" type="name" value={email} changeAction={setEmail} />
				<FormInput title="Password" type="password" value={password} changeAction={setPassword} />

				<div className="mt-2 text-gray-600">Choose if you want to log in as master or a business</div>
