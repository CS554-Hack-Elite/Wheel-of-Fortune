...
router.route("/login").post(async (req, res) => {
	try {
		...
		objKeys.forEach((element) => {
			result[element] = helpers.checkInput(element, result[element], element + " for the admin");
		});
		const adminRow = await adminData.checkAdmin(result, process.env.MASTER_ADMIN_ROLE);
...
router.route("/register-business-admin").post(async (req, res) => {
	try {
		...
		objKeys.forEach((element) => {
			result[element] = helpers.checkInput(element, result[element], element + " for the business");
		});
