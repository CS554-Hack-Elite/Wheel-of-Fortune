...
export const CustomerProof = () => {
  ...
  async function fetchBusinessList() {
    try {
      ...
      const businessArray = await axios.get(
        "/users/business-list",
        payloadHeader
      );
    ...
    if (proofList && proofList.length > 0) {
      return proofList.map((proof) => {
        return (
          <div
            key={proof.name}
            className="h-fit col-span-1 coupon bg-indigo-800 text-slate-200 rounded-lg my-4"
          >
            ...
            {proof.proof && proof.proof.length > 0 ? (
              ...
              <img
                src="https://placehold.co/3000@3x?text=No+Image+Available&font=open-sans"
                alt={proof.coupon_name}
              />
            )}
            <div className="flex justify-center pb-2 pt-2 text-xl">
              Uploaded for: {getBusinessName(proof.business_id)}
            </div>
            <div className="proofStatus px-4 py-2">
              Current status:{" "}
              {proof.status && proof.status === 1
                ? "Pending"
                : proof.status === 2
                ? "Approved"
                : proof.status === 3
            ...
            <span className="py-2">
              <div className="px-4 py-2 text-s">
                Points Earned:{" "}
                {proof.points_earned ? proof.points_earned : "None"}
              </div>
            </span>
          </div>
        );
